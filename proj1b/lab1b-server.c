...
int sockfd;
z_stream out_stream; // we want to compress before we send data across socket
z_stream in_stream; // we want to decompress when we receive data from socket
static int portFlag;
		...
		while (1) {
			...
			if (fds[0].revents & POLLIN) {
				...
				if (compressFlag) {
					...
					for (int i = 0; i < BUFFER_SIZE - (int) in_stream.avail_out; i++) {
						...
						else if (out[i] == 0x0D || out[i] == 0x0A) {
							write(pipeP2C[1], carriageReturn + 1, 1);
						}
						else {
							write(pipeP2C[1], &out[i], 1);
						}
					}
				}
				else {
			...
			if (fds[1].revents & POLLIN) {
				...
				if (compressFlag) {
					...
					send(new_socket, out, BUFFER_SIZE - out_stream.avail_out, 0);
				}
				else {
					for (int i = 0; i < nread; i++) {
